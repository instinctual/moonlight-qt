#include "stationconnectwaylandcursor.h"
...
int createAnonymousFile(size_t size)
{
    const int fd = static_cast<int>(syscall(
            SYS_memfd_create, "stationconnect-wacom-cursor", MFD_CLOEXEC));
...
class StationConnectWaylandCursor::Impl
{
public:
    struct Buffer {
...
const wl_registry_listener StationConnectWaylandCursor::Impl::RegistryListener = {
    registryGlobal,
    registryRemove,
};

const wl_buffer_listener StationConnectWaylandCursor::Impl::BufferListener = {
    bufferRelease,
};
...
StationConnectWaylandCursor::StationConnectWaylandCursor(
        std::unique_ptr<Impl> impl)
    : m_Impl(std::move(impl))
{
}

StationConnectWaylandCursor::~StationConnectWaylandCursor() = default;

std::unique_ptr<StationConnectWaylandCursor>
StationConnectWaylandCursor::create(SDL_Window* parentWindow)
{
#ifdef HAS_WAYLAND
    ...
    return std::unique_ptr<StationConnectWaylandCursor>(
            new StationConnectWaylandCursor(std::move(impl)));
#else
    (void) parentWindow;
    return nullptr;
#endif
...
void StationConnectWaylandCursor::dispatchPending()
{
#ifdef HAS_WAYLAND
...
bool StationConnectWaylandCursor::isAttachedTo(SDL_Window* parentWindow) const
{
#ifdef HAS_WAYLAND
...
void StationConnectWaylandCursor::setImage(
...
void StationConnectWaylandCursor::setPosition(int hotspotX, int hotspotY)
{
#ifdef HAS_WAYLAND
...
void StationConnectWaylandCursor::setVisible(bool visible)
{
#ifdef HAS_WAYLAND
